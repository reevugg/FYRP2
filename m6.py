...
# Initialize serial communication (adjust 'COM3' and baudrate as needed)
...
detected_qr = ""
frame = None
snapshot_interval = 100  # Interval in milliseconds (0.1 seconds)
start_time = time.time()
mode = "OCR"  # Default mode is OCR
postcode_serial_map = {}
sent_postcodes = set()  # Set to keep track of sent postcodes
...
# Function to process frame for OCR
def process_frame_ocr(snapshot):
    global detected_ocr, confidence, orientation, is_processing
    start = time.time()
    with processing_lock:
        ...
        if ocr_text:
            ...
            confidence = conf
            orientation = orient
            if confidence >= 80 and ocr_text in postcode_serial_map and ocr_text not in sent_postcodes:
                send_serial_command(postcode_serial_map[ocr_text])
                sent_postcodes.add(ocr_text)
    end = time.time()
    print(f"Processing time: {end - start:.2f} seconds")
    is_processing = False

# Function to process frame for QR code
...
def send_serial_command(command):
    with serial_lock:
        print(f"Sending serial command: {command}")
        ser.write(command.encode())

# Function to capture and process snapshots
def capture_snapshot():
    global frame, snapshot_interval, is_processing
    ret, frame = cap.read()
...
def show_frame():
    ...
    if frame is not None:
        ...
        if mode == "OCR":
            ...
            if confidence >= 80:
                high_conf_label.config(text=f"OCR with 80%+ confidence: {detected_ocr}")
            else:
                high_conf_label.config(text="OCR with 80%+ confidence: None")
        else:
...
def switch_to_qr():
    ...
    high_conf_label.pack_forget()
    qr_label.pack()
    start_capture()

# Function to add postcode and serial command to the map
def add_postcode_serial():
    postcode = postcode_entry.get()
    serial_command = serial_entry.get()
    if postcode and serial_command:
        postcode_serial_map[postcode] = serial_command
        listbox.insert(tk.END, f"Postcode: {postcode}, Serial: {serial_command}")
...
# Input fields for postcode and serial command
input_frame = tk.Frame(root)
input_frame.pack()
tk.Label(input_frame, text="Postcode:").pack(side=tk.LEFT)
postcode_entry = tk.Entry(input_frame)
postcode_entry.pack(side=tk.LEFT)
tk.Label(input_frame, text="Serial Command:").pack(side=tk.LEFT)
serial_entry = tk.Entry(input_frame)
serial_entry.pack(side=tk.LEFT)
add_button = ttk.Button(input_frame, text="Add", command=add_postcode_serial)
add_button.pack(side=tk.LEFT)

# Listbox to display postcodes and serial commands
...
high_conf_label = ttk.Label(root, text="OCR with 80%+ confidence: None", wraplength=300)
